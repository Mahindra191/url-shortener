...
@RestController
@RequestMapping // Or whatever your base route setup is
@CrossOrigin(origins = "http://localhost:5173", allowedHeaders = "*", methods = {RequestMethod.POST, RequestMethod.GET, RequestMethod.OPTIONS}) // 🎯 MASTER CORS KEY
public class UrlController {
    ...
    public ResponseEntity<?> createShortUrl(@RequestBody UrlRequest request) {
        try {
            // Send directly to service layer execution
            String shortUrlResult = urlService.shortenUrl(request);
            return ResponseEntity.ok(shortUrlResult);
        } catch (IllegalArgumentException ex) {
            // Catch standard or smart fallback errors and map them to clean JSON
    ...
    public ResponseEntity<Void> redirectUrl(@PathVariable String shortUrl) {
        // 1. Fetch the destination from our system logic
        ...
        if (longUrl == null) {
            // Fallback if the link doesn't exist or expired
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }

        // 2. Build the HTTP Redirect response headers
        HttpHeaders headers = new HttpHeaders();
        headers.setLocation(URI.create(longUrl));

        // 3. Return a 302 Found status with the redirection headers
